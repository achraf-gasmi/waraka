...
def parse_transaction(raw: dict, entities: list[Entity]) -> Optional[Transaction]:
    ...
    try:
        ...
        return Transaction(
            ...
            structuring_below_threshold=bool(raw.get("structuring_below_threshold", False)),
            multiple_transactions_same_beneficiary=bool(
                raw.get("multiple_transactions_same_beneficiary", False)
            ),
            no_commercial_justification=bool(raw.get("no_commercial_justification", False)),
            weak_aml_destination=bool(raw.get("weak_aml_destination", False)),
            activity_inconsistency=bool(raw.get("activity_inconsistency", False)),
        )
