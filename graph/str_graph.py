...
from graph.rules_insurance import compute_jurisdiction_flags, score_insurance_case
from models.schemas import Entity, Transaction, RiskLevel
from tools.fatf_lookup import FATF_COUNTRY_WEIGHTS
...
def assess_risk_node(state: STRState) -> dict:
    ...
    if sector == "assurance":
        case: dict = dict(state.get("extracted_transaction", {}))
        case.update(compute_jurisdiction_flags(case))
